...
export async function GET(_req: NextRequest) {
  ...
  if (!latestLog?.topicId || !latestLog.postContent)
    return NextResponse.json({ error: 'No successful daily prayer post found' }, { status: 404 });
  ...
  return NextResponse.json({ ok: true, topic: topic.topic, verseReference: topic.verseReference });
}
