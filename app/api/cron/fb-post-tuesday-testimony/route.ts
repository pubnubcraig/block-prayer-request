...
export async function GET(req: NextRequest) {
  ...
  const db = getDb();
  if (!db) {
    console.error('[fb-post-tuesday-testimony] Database unavailable');
    return NextResponse.json({ error: 'DB unavailable' }, { status: 503 });
  }

  // ── 3. Duplicate check — skip if testimony already posted today
    ...
    .where(
      and(
        ...
        sql`${facebookPostLog.topicId} IN (
          SELECT id FROM engagement_topics WHERE content_type = 'testimony'
        )`,
      ),
    )
    .limit(1);
  ...
  if (existingPost) {
    console.log('[fb-post-tuesday-testimony] Already posted today, skipping');
  ...
  try {
    // ── 4. Select testimony topic ──────────────────────────────────
    const topic = await selectEngagementTopic({ include: 'testimony' });
    topicId = topic.id;
    console.log(
      `[fb-post-tuesday-testimony] Selected topic: "${topic.contentType}" (${topic.id})`,
    );
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        console.log(
          `[fb-post-tuesday-testimony] Published to Facebook: ${facebookPostId}`,
        );
        break;
      } catch (fbError) {
        lastFbError =
        ...
        console.warn(
          `[fb-post-tuesday-testimony] Facebook attempt ${attempt}/3 failed:`,
          lastFbError.message,
        );
        if (attempt < 3) {
    ...
    if (!facebookPostId) {
      ...
      await notifyPostFailure(
        ...
        console.error(
          '[fb-post-tuesday-testimony] Failed to send failure notification:',
          e,
        ),
      );
    ...
    console.error('[fb-post-tuesday-testimony] Unhandled error:', message);
    ...
    try {
      ...
      console.error('[fb-post-tuesday-testimony] Failed to log error:', logErr);
    }
